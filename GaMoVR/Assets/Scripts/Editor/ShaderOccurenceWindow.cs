...
using System.IO; public class ShaderOccurenceWindow : EditorWindow
{
    [MenuItem("Tools/Shader Occurence")] public static void Open()
    {
    ...
    void OnGUI()
    {
        ...
        if (shader != prev)
        {
            ...
            materials.Clear();
            for (int i = 0; i < allMaterials.Length;
            i++) {
            ...
            for (int i = 0; i < materials.Count; i++)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Show"))
                        EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath(materials[i],
                            typeof(Material)))
                    ;
                }
